...
export const useCreatePost = () => {
  ...
  const createPostMutation = useMutation({
    ...
    onError: () => {
      Alert.alert('Error', 'Failedto create post.Please try again');
    },
  });
  ...
  const handleImagePicker = async (useCamera: boolean = false) => {
    ...
    if (
      ...
      resolvedResult.assets &&
      resolvedResult.assets.length > 0
    ) {
      setSelectedImage(resolvedResult.assets[0].uri);
    }
  };
  ...
  return {
    ...
    pickImageFromGalery: () => handleImagePicker(false),
