...
const MessagesScreen = () => {
  ...
  const [converstionsList, setConverstionsList] = useState(CONVERSATIONS);
  ...
  const deleteConversation = (conversationId: number) => {
    Alert.alert(
      'Delete conversation',
      'Are you sure you want to delet conversation?',
      [
  ...
  const sendMessage = () => {
    if (newMessage.trim() && selectedConversation) {
      // upldate last message in conversation
      setConverstionsList((prev) =>
        prev.map((conv) =>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={['top']}>
      ...
      >
        {converstionsList.map((conversation) => (
          <TouchableOpacity
            key={conversation.id}
        ...
        {selectedConversation && (
          <SafeAreaView className="flex-1">
              ...
              <View className="mb-4">
                ...
                {selectedConversation.messages.map((message) => (
                  ...
                  >
                    ...
                    >
                      <View
                        className={`rounded-2xl px-4 py-3 max-w-xs ${message.fromUser ? 'bg-blue-500' : 'bg-gray-500'}`}
                      >
                        <Text
                          className={
            ...
            <View className="flex-row items-center px-4 py-3 border-t border-gray-100">
              <View className="flex-1 flex-row items-center rounded-full px-4 py-3 bg-gray-100">
                <TextInput
